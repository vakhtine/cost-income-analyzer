...
export function CityCompareGrid({ summaries, formatAmount, displayCurrency }: Props) {
  ...
  const rankedScores = applyRecommendationRankScores(
    sorted.map((entry) => ({
      city: entry.city,
  ...
  const scoreByCity = new Map(rankedScores.map((entry) => [entry.city, entry.score]));
    ...
    <section className="card city-compare-grid">
      <div className="section-heading">
        ...
        <p>
          ...
          and lifestyle adjustments. Only the top-ranked city scores 100/100.
        </p>
      </div>
      <div className="city-compare-cards">
        {sorted.map((entry, index) => {
          ...
          const displayScore = scoreByCity.get(entry.city) ?? affordability.score;
