...
function pageShell(
  ...
  return buildReportPageShell({
    pageNumber,
    totalPages,
    reportLabel: "CUSTOM FINANCIAL REPORT",
    pageTitle: title,
    body,
    privacyNotice,
  });
...
function buildExpensesReport(payload: CustomReportPayload, pageNumber: number, totalPages: number) {
  const { periodAnalysis, periodLabel, generatedAt, displayCurrency, data } = payload;
  ...
  const body = `
    <h1 class="main-title">Expenses by category</h1>
    <p class="intro">
      Generated ${escapeHtml(generatedAt)}. ${escapeHtml(expenseDateLabel)}.
      All amounts converted to ${escapeHtml(displayCurrency)}.
    </p>
    <hr class="divider" />
    ...
    <p class="section-note">${escapeHtml(expenseDateLabel)}</p>
...
function buildHealthReport(payload: CustomReportPayload, pageNumber: number, totalPages: number) {
  const { health_score } = payload.data;
  ...
  const body = `
    <h1 class="main-title">Financial health score</h1>
    <p class="intro">
      Generated ${escapeHtml(payload.generatedAt)} using latest period data (${escapeHtml(payload.periodLabel)}).
      Overall score combines savings rate (40%), income stability (30%), and non-essential spending (30%).
      This score reflects your spending habits — it is separate from relocation affordability (city cost fit).
    </p>
    <hr class="divider" />
    <div class="score-ring score-ring-${tone}">${health_score.overall}</div>
    <p class="section-note score-summary score-summary-${tone}" style="text-align:center;margin-bottom:4mm;">
      <strong>${escapeHtml(health_score.summary)}</strong>
    </p>
    ${metricsHtml}
    <h2 class="section-title">Score breakdown</h2>
    <p class="section-note">
      Overall score = (Savings rate × 40%) + (Income stability × 30%) + (Non-essential control × 30%).
...
function buildBestFitReport(payload: CustomReportPayload, pageNumber: number, totalPages: number) {
  const { recommendations, baseCity, periodLabel, generatedAt, displayCurrency } = payload;

  if (!recommendations.length) {
    const body = `
      <h1 class="main-title">Best-fit cities by budget</h1>
      <p class="intro">Generated ${escapeHtml(generatedAt)}. Run city comparison in the app to populate rankings.</p>
      <p class="section-note">No city recommendations available yet.</p>
    `;
    return pageShell(
      pageNumber,
      totalPages,
  ...
  const body = `
    <h1 class="main-title">Best-fit cities by budget</h1>
    <p class="intro">
      Generated ${escapeHtml(generatedAt)} for period ${escapeHtml(periodLabel)}.
      ${baseCity ? `Compared against your spending from ${escapeHtml(baseCity)}.` : ""}
      Ranked by projected monthly balance in ${escapeHtml(displayCurrency)}.
    </p>
    <hr class="divider" />
    <table class="index-table">
      <thead>
...
export function buildCustomReportsHtml(
  ...
  const pages = types.map((type, index) => {
    ...
    const totalPages = types.length;
    if (type === "expenses-by-category") return buildExpensesReport(payload, pageNumber, totalPages);
    if (type === "financial-health") return buildHealthReport(payload, pageNumber, totalPages);
    return buildBestFitReport(payload, pageNumber, totalPages);
  });

  const title = `Custom report — ${types.map((type) => CUSTOM_REPORT_LABELS[type]).join(", ")}`;
